...
local GLOBPATH = "/keymaps/";

-- bitmask translation (constants not exposed to lua by engine)
MOD_NONE   = 0x0000;
MOD_LSHIFT = 0x0001;
MOD_RSHIFT = 0x0002;
MOD_LCTRL  = 0x0040;
...
symtable.temp_prefix = "";

symtable.patch = function(tbl, iotbl)
	iotbl.modifers = iotbl.modifiers or 0;
...
-- other symbols are described relative to the internal sdl symbols
	...
	local lutsym = symtable.temp_prefix .. (string.len(mods) > 0 and
			(mods .."_" .. sym) or sym);
	symtable.temp_prefix = "";
...
-- two support layers at the moment, normal press or with modifiers.
	...
	if (DEBUGLEVEL > 0) then
		print(CLOCK, "symtable:patch", sym, lutsym);
	end
